...
def test_get_enforcement(decisions_gold, options_test):
    decisions = decisions_gold.copy()
    extras.get_enforcement(decisions, options_test)
...
def test_get_decision_citation(decisions_extras_gold, decisions_extra_mock, options_test):
    extras.get_decision_citation_all(decisions_extras_gold, options_test)
    assert decisions_extras_gold[0].citation == '[2020] SGPDPC 1'
    assert decisions_extras_gold[0].case_number == 'DP-1234-5678A'


def test_get_case_references(decisions_extra_mock, options_test):
    import pickle
    import os
    decisions = pickle.load(open(os.path.join(os.getcwd(), 'tests', 'decisions_references.pickle'), 'rb'))
    extras.get_decision_citation_all(decisions, options_test)
    extras.get_case_references_all(decisions, options_test)
...
def test_scraper_extras(decisions_gold, options_test):
    assert extras.scraper_extras(decisions_gold, options_test)
