...
def test_get_decision_citation(decisions_gold):
    extras.get_decision_citation_all(decisions_gold)
...
def test_get_case_references(requests_mock):
    import pickle
    import os
    decisions = pickle.load(open(os.path.join(os.getcwd(), 'tests', 'decisions_references'), 'rb'))
